...
2. ★수치 무결성(가장 중요 — 어기면 허위광고): 본문·제목·표·FAQ·메타설명에 나오는
   **모든 구체적 숫자**(광고비·매출·ROAS·비율·증감률·금액·기간 등)는 **위 근거에 실제로 적힌 값만**
   그대로 쓴다.
   - 근거에 없는 숫자는 절대 만들지 마라. 근사·반올림·과장으로 더 극적인 값으로 바꾸지 마라.
   - "75% 절감", "50% 증가" 같은 증감률·계산값은 근거에 그 수치가 명시돼 있을 때만 쓴다(직접 계산 금지).
   - 근거상 추세가 작거나 반대면(예: 광고비가 오히려 늘었으면) **있는 그대로 정직하게** 써라.
   - 근거에 없는 값이 필요하면 숫자 없이 정성적으로("조금", "크게", "안 빠졌다")만 쓰거나 생략하라.
3. 근거에 없는 **제품 기능·효과를 지어내지 마라**. 도구/제품이 실제로 하는 일만 언급하고, 확실치 않으면 언급하지 마라.
4. 익명 처리: 회사명/상품명/카테고리 등 업체를 특정할 정보를 노출하지 마라.'''
