...
def generate_shorts_script(
    ...
    # ── 제품 리빌 모드 (3씬 PAS) ────────────────────────────────
    # ── 공통 지표 단위 가이드 (3씬/5씬 공유) ──
    _KR_METRIC_GUIDE = (
        '\n\n[한국 마케팅 지표 단위 — 반드시 준수]\n'
        '• ROAS: 한국에서는 % 단위 사용 (ROAS 500% = 5배 매출).\n'
        '  입력이 배수(3, 5 등)여도 → "300%", "500%", 또는 "3배", "5배"로 표현.\n'
        '  "ROAS가 안 좋다" ≒ 100~200%. "ROAS가 좋다" ≒ 400~600%.\n'
        '• CPC: 원(₩) — "클릭당 800원" / CTR: % — "클릭률 2.5%"\n'
        '• 광고비·매출: 원(₩), 억/만 단위 — "월 광고비 500만 원", "매출 2억"'
    )
